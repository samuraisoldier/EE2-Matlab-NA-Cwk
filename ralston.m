function [t,Vout] = ralston(func,t0,i0,tf,h,Vin,R)
...
for j=1:N-1                                % loop for N-1 steps to find Vout(1)...Vout(N-1)
    k1=h*feval(func,ta(j),ia(j),Vin(j));   % gradient at t
    k2=h*feval(func,ta(j)+2*h/3,ia(j)+2*k1/3,Vin(j));
    % calculate gradient at t+(2h/3)
    ia(j+1)=ia(j)+(k1+3*k2)/4;             % next value of i calculated from previous values of t,i
